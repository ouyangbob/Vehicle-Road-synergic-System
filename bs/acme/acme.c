...
unsigned char g_testverno_magic = 'Q';  // Magic test message proto version number
...
static int parse_opts(int argc, char *argv[])
{
...
#endif
    proxy_if_name = (char *)default_proxy_if_name;
    ...
    while ((c = getopt(argc, argv,
        ...
        case 'g':
            ...
            break;
        case 'G':
            g_strlcpy(server_ip_str, optarg, sizeof(server_ip_str));
            if (gVerbosity) {
                printf("Setting kinematics server ip to %s\n", server_ip_str);
            }
            break;
        case 'i':
            iface = optarg;
...
int mainOld(int argc, char **argv)
{
    ...
    g_strlcpy(server_ip_str, ip, strlen(ip) + 1);
    freeaddrinfo(result);
    ...
    if (using_tx_timer) {
        ...
        if (gVerbosity && interval_btw_pkts_ns) {
        ...
        if (test_mode == ModeEcho) {
            ...
            if (test_mode != ModeRx) {
                ...
                if (do_net2air_proxy) {
                    ...
                    demo_payload_tx_setup(buf, g_tx_seq_num++, payload_len[paynum],
                                          timestamp, NULL);
                    ...
                    sample_tx(tx_sock, payload_len[paynum]);
